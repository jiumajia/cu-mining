...
class Wind_Data(object):
    ...
    def fill_data(self,col_name,method):
        ...
        if method == 'median':
            self.dataset[col_name]= self.dataset[col_name].fillna(self.dataset[col_name].median()) #汇率用均值填充
        elif method == 'mean':
            self.dataset[col_name]= self.dataset[col_name].fillna(self.dataset[col_name].mean()) #汇率用中位数填充
        elif method == 'bfill':
            self.dataset[col_name] = self.dataset[col_name].fillna(method='bfill')  #库存用周五的数据填充
        elif method == 'pad':
            self.dataset[col_name] = self.dataset[col_name].fillna(method='pad')  #库存用周五的数据填充
    ...
    def set_features(self):
        ...
        p_data['d711'] = self.dataset['PE100058'] - self.dataset['MA000001']
        ...
        p_data['day']  = self.dataset['date'].map(get_day)
        #index
